...
let db, col, seg, forn, comp, sess, ped
...
beforeEach(() => {
  ...
  compId = comprador.id
  const sessao = sess.create({ fornecedor_id: fornId, colecao_id: colId, data_visita: '2026-05-16', vendedor: '', cond_pag: '', frete: '', obs: '' }, [compId])
  visitaId = sessao.visitas[0].visita_id
})
...
describe('pedidos', () => {
  ...
  it('accumulates quantities from multiple orders for same seg+col+size', () => {
    ped.salvar({ visita_id: visitaId, comprador_id: compId, segmentacao_id: segId,
      valor_unitario: 49.90, itens: [{ tamanho: 'P', qtd: 20 }]
    })
    ped.salvar({ visita_id: visitaId, comprador_id: compId, segmentacao_id: segId,
      valor_unitario: 49.90, itens: [{ tamanho: 'P', qtd: 15 }]
    })
    const totais = ped.totaisPorTamanho(segId, colId)
  ...
  it('returns all orders for a visit', () => {
    ped.salvar({ visita_id: visitaId, comprador_id: compId, segmentacao_id: segId,
      valor_unitario: 50.00, itens: [{ tamanho: 'P', qtd: 10 }, { tamanho: 'M', qtd: 20 }]
    })
  ...
  it('returns all orders with comprador info when multiple buyers order from same visit', () => {
    ...
    ped.salvar({ visita_id: visitaId, comprador_id: compId, segmentacao_id: segId,
      valor_unitario: 49.90, itens: [{ tamanho: 'P', qtd: 20 }, { tamanho: 'M', qtd: 30 }]
    })
    ped.salvar({ visita_id: visitaId, comprador_id: comp2.id, segmentacao_id: segId,
      valor_unitario: 49.90, itens: [{ tamanho: 'P', qtd: 10 }]
    })
    const pedidos = ped.byVisita(visitaId)
