...
export default function Vagas({ navigate }) {
  ...
  async function toggleStatus(vaga) {
    const novoStatus = vaga.status === 'aberta' ? 'pausada' : 'aberta'
    await fetch('/api/vagas', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: vaga.id, status: novoStatus }),
    })
    load()
  }
          ...
          <div style={s.row}>
            ...
            <label style={s.label}>
              ...
              <input value={form.local} onChange={e => setForm(f => ({ ...f, local: e.target.value }))}
                style={s.input} placeholder="Ex: Porto Alegre" />
            </label>
          </div>
          <label style={s.label}>
            Descrição
            ...
            <div key={v.id} style={s.card}>
              <div style={s.cardMain} onClick={() => navigate('vaga-detalhe', { vagaId: v.id, vagaTitulo: v.titulo })}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  {v.local && <span style={s.location}>📍 {v.local}</span>}
                </div>
              ...
              <div style={s.cardActions}>
                <button onClick={() => toggleStatus(v)} style={s.btnSm}>
                  {v.status === 'aberta' ? 'Pausar' : 'Reabrir'}
                </button>
                <button onClick={() => navigate('vaga-detalhe', { vagaId: v.id, vagaTitulo: v.titulo })} style={s.btnSm}>
                  Ver kanban →
                </button>
              </div>
            </div>
...
const s = {
  ...
  btnPrimary:{ padding: '9px 18px', borderRadius: 8, border: 'none', background: '#1e293b', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' },
  btnSm:     { padding: '6px 12px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', fontSize: 13, cursor: 'pointer', color: '#374151' },
  formCard:  { background: '#fff', borderRadius: 12, padding: 24, marginBottom: 24, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: 16 },
  ...
  input:     { padding: '9px 12px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 14, outline: 'none', fontFamily: 'inherit' },
  error:     { margin: 0, color: '#dc2626', fontSize: 13 },
  list:      { display: 'flex', flexDirection: 'column', gap: 12 },
  card:      { background: '#fff', borderRadius: 12, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 },
